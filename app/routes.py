...
def set_book_as_rented(user,book_id):
   ...
   return BookService.rent_book(data,db, user['id'])
...
def get_rented_book(user, book_id):
   data = HistoryService.get_rented_book(book_id, user['id'])
   return HistoryService.return_book(data, db, user['isAdmin'])
...
def get_rentals(user):
   ...
   if not user['isAdmin']:
      return send_response("adminerror")
